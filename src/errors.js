...
export const invalidBreakpointNamesErrorMessage = breakpointMap =>
  `You supplied a breakpoint set with an invalid name. Valid values are: '${rangedFeatureNames}', but you supplied: '${objectToString(
    breakpointMap
  )}'.`
...
export const sameBreakpointsForBetweenErrorMessage = name =>
  `You must supply two different breakpoints to 'widthBetween' but both were: '${name}'.`
...
export const queryElementIsValidTypeErrorMessage = value =>
  `You must only supply strings or arrays to 'query()' but you supplied '${value}'.`
...
export const queryNoNestedArraysErrorMessage = value =>
  `You must not supply any nested arrays as part of a query but you supplied '${value}'.`
