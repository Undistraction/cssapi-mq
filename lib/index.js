...
var breakpointsWereSupplied = both(complement(isEmpty), is(Object));
var breakpointValuesAreValid = compose(all(is(Number)), values);
var baseFontSizeIsValid = both(is(Number), gt(__, 0));
var defaultMediaTypeIsValid = contains$1(__, values(MEDIA_TYPES));
var unitIsValid = contains$1(__, values(UNITS));
...
var validateBreakpoints = function validateBreakpoints(breakpoints) {
  ...
  if (!breakpointValuesAreValid(breakpoints)) throw new Error('You must supply unitless values for each breakpoint but you supplied ' + values(breakpoints));
};
...
var validateConfig = function validateConfig(_ref) {
  ...
  if (!defaultMediaTypeIsValid(defaultMediaType)) {
    throw new Error('\'defaultMediaType\' must be one of \'' + values(MEDIA_TYPES) + '\' but was \'' + defaultMediaType + '\'');
  }

  if (!unitIsValid(unit)) {
    throw new Error('\'unit\' must be one of \'' + values(MEDIA_TYPES) + '\' but was \'' + unit + '\'');
  }
...
var configure = function configure(breakpoints) {
  ...
  var toBreakpointArray = compose(map(zipObj(['name', 'value'])), toPairs);
  var orderByValue = compose(reverse, sort(prop('value')));
  ...
  var getUpperLimit = function getUpperLimit(breakpoint) {
    var index = findIndex(propEq('name', breakpoint))(breakpointsArray);
  ...
  var missingBreakpointErrorMessage = function missingBreakpointErrorMessage(name) {
    return 'There is no breakpoint defined called \'' + name + '\', only: \'' + keys(breakpoints) + '\' are defined.';
  };
  ...
  var buildQuery = function buildQuery(definition, content) {
    return css(_templateObject, definition, content);
  };
  ...
  var aboveWidth = function aboveWidth(from) {
    ...
    return function (stringParts) {
      ...
      return buildQuery(buildQueryDefinition(config.mediaType, minWidth(from)), css.apply(undefined, [stringParts].concat(interpolationValues)));
    };
  };
  ...
  var belowWidth = function belowWidth(to) {
    ...
    return function (stringParts) {
      ...
      return buildQuery(buildQueryDefinition(config.mediaType, maxWidth(to)), css.apply(undefined, [stringParts].concat(interpolationValues)));
    };
  };
  ...
  var betweenWidths = function betweenWidths(from, to) {
    ...
    return function (stringParts) {
      ...
      return buildQuery(buildQueryDefinition(config.mediaType, minWidth(from), maxWidth(to)), css.apply(undefined, [stringParts].concat(interpolationValues)));
    };
  };
